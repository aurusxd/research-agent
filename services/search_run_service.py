...
class SearchRunService:
    ...
    def _build_agent_task(
        ...
        parts = [
            ...
            (
                "Не сохраняй организацию, если не найден хотя бы один "
                "реальный канал связи: email, официальная форма, VK, "
                "Telegram или Одноклассники. Сайт, YouTube, Rutube и Дзен "
                "без контактных данных каналом связи не считаются. Для "
                "каждого сохраняемого контакта обязательно выбери "
                "preferred_channel и передай адрес получателя этого канала. "
                "Перед выбором contact_form проверь, что на странице есть "
                "интерактивное поле сообщения и кнопка отправки. Страница "
                "с опубликованным email без формы должна сохраняться как "
                "канал email."
            ),
            (
